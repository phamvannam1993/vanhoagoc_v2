...
const rawGroups = [
    ...
    { label: 'Giảng dạy', items: [
        { name: 'Giao bài',  icon: 'send',   link: route('admins.teaching.assign'), match: ['admins.teaching.assign','admins.practices','admins.class.assignment'], roles: [USER_TYPE_DIRECTOR, USER_TYPE_TEACHER] },
...
const submitLogin = () => {
    ...
    // Inertia sẽ tự động handle redirect nếu thành công
    router.post(route('login'), {
        ...
        onSuccess: () => {
            // Login thành công - Inertia sẽ tự động redirect
