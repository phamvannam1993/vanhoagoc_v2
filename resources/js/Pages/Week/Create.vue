...
const numberWeek = page.props.numberWeek;
...
const rules = {
    ...
    numberWeek: yup.string().required("ID tuần không được để trống")
};
...
const handleSubmit = () => {
    ...
    axios
        .post(route("weeks.json.store"), formData)
        .then((response) => {
            if (response.data.status) {
                toast.success("Tạo tuần thành công");
                setTimeout(function() {
...
const goBack = () => {
    window.location = route("weeks.index", { app_id: app_id, book_id: book_id });
};
</script>

<template>
    <Head title="Create Week" />
    ...
    <MasterLayout>
        ...
        <div class="app-page py-4">
            <div class="content-page mx-auto w-4/5 sm:px-6 lg:px-8">
                <h1 class="text-[30px] font-bold text-[#2C75E3]">Thêm mới tuần</h1>
                <Form @submit="handleSubmit">
                    ...
                    <div class="relative mt-6 w-4/5">
                        ...
                        <div class="mt-2 w-4/5">
                            <Field name="numberWeek" :rules="rules.numberWeek" v-model="form.numberWeek">
                                <a-input
                                    placeholder="ID tuần"
                                    :allow-clear="true"
                            ...
                            <ErrorMessage class="text-sm text-red-600" name="numberWeek" />
                            <InputError class="mt-2" :message="form.errors.numberWeek" />
                        </div>
                    </div>
                    <div class="relative mt-6 w-4/5">
                        <label
