...
const loadData = async () => {
  ...
  const res = await axios.get(route('users.json.jsonListMember', params));
...
<template>
  ...
  <MasterLayout>
    <template #header>
      <h2 class="text-xl font-semibold leading-tight text-gray-800">Member</h2>
    </template>

    <div class="app-page py-4">
      <div class="content-page mx-auto w-full md:w-10/12">
        <h1 v-if="type_id === 2" class="text-[30px] font-bold text-[#2C75E3]">
          Thành viên Admin
        </h1>
        <h1
          v-else-if="type_id === 3"
          class="text-[30px] font-bold text-[#2C75E3]"
        >
          Thành viên Biên tập
        </h1>
        <h1 v-else class="text-[30px] font-bold text-[#2C75E3]">Thành viên</h1>
        <a-button @click="goBack" class="custom-bg text-black" size="large">
          Quay lại
        </a-button>
        <Link
          :href="route('users.newMember', { type_id: type_id })"
          class="ml-4 mt-6"
        >
          <a-button class="mt-6" type="primary" size="large"
            >Thêm tài khoản
          </a-button>
        </Link>
        <div class="mt-4">
          ...
          >
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-if="column.key === 'app_names'">
                <div class="flex flex-col gap-1">
                  <a-tag v-for="appName in record.app_names" :key="appName" color="blue">{{ appName }}</a-tag>
                  <span v-if="!record.app_names.length" class="text-gray-400">—</span>
                </div>
              </template>
