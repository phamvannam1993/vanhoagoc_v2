...
const infoTemplate = ref(props.record.template);
const optionsType = [
    { label: 'Text', value: 1 },
    { label: 'Ảnh', value: 2 },
...
const form = useForm({
  id: props.record.id,
  code: props.record.playable,
  type: props.record.type,
  type_label: props.record.type_label,
  template: {
    name: infoTemplate.value.name,
    number_answer: infoTemplate.value.number_answer,
    number_answer_connect: infoTemplate.value.number_answer_connect,
    max_length_question: infoTemplate.value.max_length_question,
    sub_question: infoTemplate.value.sub_question,
    reading: infoTemplate.value.reading,
    video: infoTemplate.value.video,
    audio: infoTemplate.value.audio,
    preview: infoTemplate.value.preview,
    max_length_answer: infoTemplate.value.max_length_answer,
    add_answer: infoTemplate.value.add_answer,
    type_answer: infoTemplate.value.type_answer,
    type_question: infoTemplate.value.type_question
  },
  app_id: parseInt(app_id),
  image_show: props.record.image_show,
  image_db: props.record.image,
  image_name: '',
  ...
  axios
    ...
    .then((response) => {
      if (response.data.status) {
        toast.success('Chỉnh sửa template thành công');
        setTimeout(function () {
...
const options = ref(
  Object.entries(props.listType).map(([value, label]) => ({
    value,
    label,
  })),
);
  ...
  <MasterLayout>
    ...
    <div class="app-page mx-auto flex w-4/5 gap-4 py-4">
      <div class="content-page w-full">
        <h1 class="text-[30px] font-bold text-[#2C75E3]">Chỉnh sửa template</h1>
