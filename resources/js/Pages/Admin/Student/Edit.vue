...
onMounted(async () => {
    const params = {
        app_id: props.user.student_app[0]?.id
    };
...
const result = ref(props.user.classes.map(item => item.id));
const form = useForm({
    id:props.user.id,
    ...
    tel: props.user.tel,
    app_id: props.user.student_app[0]?.id,
    class_id: props.user?.classes ? parseInt(props.user?.classes[0]?.id) : null
});
const rules = {
    password: yup.string(),
    ...
    axios
        ...
        .then((response) => {
            if (response.data.status) {
                toast.success('Cập nhật tài khoản Nhân viên/Học sinh thành công');
                setTimeout(function () {
                    if (class_id) {
    ...
    <SchoolLayout>
        ...
        <div class="app-page mx-auto flex w-4/5 gap-4 py-4">
            <div class="content-page w-full">
                <h1 class="text-[30px] font-bold text-[#2C75E3]">Sửa Nhân viên/Học sinh</h1>
