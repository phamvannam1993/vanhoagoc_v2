...
<template>
    <a-modal v-model:open="open" @ok="handleOk" :width="1000">
        ...
        <div class="flex items-center gap-4 mt-4">
            <div class="text-[16px] h-max flex justify-center w-full rounded-md border-2 border-solid border-[#E5E5E5] bg-white p-4 max-md:max-w-full">
                <div class="w-full" v-if="data.question_type === 1 || data.question_type === 3">
                    <div>
                        {{ data.question || 'Bạn nhập nội dung hoặc tải ảnh ở đây' }}
                    </div>
                    <audio
                        class="mt-4"
                        v-if="data.question_type === 3 && data.audio_question_show"
                        ref="audioPlayer"
                        :src="data.audio_question_show"
                        controls
                    ></audio>
                </div>
                <div v-else>
                    <img
                        v-if="data.question_type === 2 && data.image_question_show"
                        :src="data.image_question_show"
                        alt="Preview"
        ...
        <div class="mt-4 grid w-full gap-4 grid-cols-3 md:grid-cols-3 lg:grid-cols-3">
            <div
                v-for="(item, index) in data.listAnswer"
                :key="index"
                class="flex min-h-[54px] w-full items-center justify-center rounded-md border-2 border-solid border-[#E5E5E5] bg-white text-center max-md:max-w-full"
            >
                <div class="w-full justify-items-center" v-if="item.type_answer === 1 || item.type_answer === 3">
                    <div class="my-4">
                        {{ item.inputAnswer || ''}}
                    </div>
                    <audio
