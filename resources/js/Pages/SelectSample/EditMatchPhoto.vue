...
const form = useForm({
  ...
  image_question_show: props.data.question_val_show || '',
  image_question_db: props.data.question_val || '',
...
const rules = {
  ...
  image_question_db: yup.string().required('Ảnh không được để trống'),
};
const handleSubmit = () => {
...
const handleFileChangeQuestion = async (event) => {
  ...
  if (!isImage) {
    ...
    try {
        ...
        form.file_reading_name = file.name;
    } catch (error) {
        console.error("Lỗi upload:", error);
    } finally {
        isUploading.value = false;
...
<template>
  ...
  <MasterLayout :breadcrumbs=breadcrumbs>
              ...
              >
                ...
                <div class="custom-gap mt-8 w-4/5">
                  <div class="mb-4 flex justify-center">
                    <div v-if="form.image_question_show">
                      <img
                        :src="form.image_question_show"
                        @click="triggerFileInputQuestion"
                        alt="Preview"
                        class="h-[300px] w-[300px] cursor-pointer object-fill"
                      />
                      <img
                    ...
                    <img
                      v-else
                      @click="triggerFileInputQuestion"
                      src="/images/games/match-photo/icon-match-photo.png"
                      alt="image-select"
                      class="cursor-pointer"
                    />
                  </div>
                  ...
                  <div class="mb-8 mt-4 flex justify-center text-center">
                      <div class="text-sm italic text-[#2C75E3]">(Tốt nhất hãy chọn một bức ảnh hình vuông)</div>
                    <input
                      id="file-input"
                      type="file"
                      accept="image/*"
                ...
                <Field
                  name="image_question_db"
                  class="hidden"
                  :rules="rules.image_question_db"
                  v-model="form.image_question_db"
                >
                </Field>
                <ErrorMessage
                ...
                <InputError
                  class="mt-2"
                  :message="form.errors.image_question_db"
                />
    ...
    <a-modal v-model:open="openPreview" @ok="handleOkPreview" :width="1000">
      ...
      <div class="flex gap-2">
        ...
        <div v-if="form.audio_title_show" class="flex items-center">
          <img
            class="h-[30px] w-[30px] cursor-pointer"
            src="/images/icon-sound.png"
            alt=""
        ...
        <div v-if="form.video_title_name" class="flex items-center gap-2">
          <img
            @click="openVideoTitle"
            class="h-[30px] w-[30px] cursor-pointer"
            src="/images/icon-game-choose-correct/icon-video-new.png"
          />
          <div>{{ form.video_title_name }}</div>
        </div>
      </div>
      <div
      ...
      >
        <img
          class="relative h-[300px] w-[300px] object-cover"
          :src="form.image_question_show"
          alt=""
        />
    ...
    <a-modal v-model:open="openVideo" :width="800">
      <div class="flex justify-center">
        <div v-if="form.video_title_show">
          <div v-if="form.isYouTubeVideo">
            <!-- Nhúng video YouTube -->
            <iframe
              :src="form.video_title_show"
              frameborder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen
              width="600"
